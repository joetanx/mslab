...
#### 1.3.1. Tier 1: Alert Analysts

Tier 1 analysts are typically the first responders, focusing on triaging and initial analysis of alerts. They need tools that help them quickly understand the nature and severity of alerts to determine the next steps.

> Example: An alert for failed logins might be checked against geo-location or frequency using KQL to determine if it’s worth escalating.

- **Monitor Incidents Blade**: Use the Sentinel dashboard to view real-time incidents generated by analytics rules.
- **Review Alerts**: Drill down into alerts for more context (IP addresses, users, endpoints).
- **Visualize with Workbooks**: Use dashboards for situational awareness (e.g., visualizing endpoint or network anomalies).
- **Playbooks for Routine Security Tasks**: Trigger automated responses (e.g., sending notifications, enriching alerts with threat intel).
- **Incident Management for Comment & Escalate**: Add notes, change severity/status, and escalate to Tier 2 if needed.

#### 1.3.2. Tier 2: Incident Responders

Tier 2 analysts perform more in-depth investigations and handle the response and remediation of security incidents. They need tools for deeper analysis, containment, and eradication.

> Example: An incident shows a user logged in from two countries within minutes; Tier 2 uses KQL to check sign-in history and disable the account.

- **Investigate Incidents**: Use the **Investigation Graph** to visualize relationships between entities (users, devices, IPs) to analyze lateral movement or attack chains.
- **KQL Queries for Investigation**: Run deeper queries in **Log Analytics** to find lateral movement, privilege escalation, etc.
- **Playbooks for Response and Containment**: Use or customize **Playbooks** to isolate endpoints (via Defender), disable user accounts (via Azure AD), etc.
- **Threat Intelligence for Investigation**: Correlate incidents with IOCs from threat intel feeds integrated into Sentinel.
- **Incident Management for Investigation Findings**: Add investigation notes, evidence, and RCA into the incident timeline.

#### 1.3.3. Tier 3: Threat Hunters

Tier 3 analysts, or threat hunters, proactively search for hidden or advanced threats that might evade standard detection mechanisms. They need powerful tools for deep data exploration, anomaly detection, and understanding attacker tactics, techniques, and procedures (TTPs).

> Example: Running a hunting query to find command line activity launching `rundll32` with network connections and then creating a rule if it matches attacker TTPs.

- **Hunting Blade**: Run advanced queries using KQL to look for suspicious activity across data sources (e.g., rare process executions, suspicious PowerShell usage).
- **Bookmarks**: Save interesting events to use in investigations or correlate with incidents.
- **Notebook Integration (Jupyter + KQL + Python)**: Develop custom threat hunting workflows, using ML and anomaly detection to perform deeper forensic analysis or enrichment.
- **Custom Analytics Rules**: Convert hunting findings into detections.
- **Threat Intelligence for Threat Hunting**: Use threat intel feeds integrated into Sentinel to pivot and hunt for associated activities (e.g., find traffic to known C2 domains).
- **Watchlists**: Maintain lists of known bad or watchworthy items (e.g., VIP users, high-risk assets).
- **MITRE ATT&CK Mapping**: Align queries with ATT&CK tactics to ensure coverage across threat categories.
- **Create Detections**: Convert successful hunts into new **Analytics Rules** to improve detection coverage.
